    ...
    def combatEntry(self, target):
        try: # this shit barely works
            ...
            action = input((f'1. ATTACK\t2. DEFEND\n'
                            f'3. BAG\t\t4. DODGE'))
            if action.lower() in ['1','2','3','4', 'attack','defend','bag','dodge']:
                print('Checking')
                if action.lower() in ['1', 'attack']:
                    print("Queueing attack")
                    self.dealdmg(target=target)
                elif action.lower() in ['2', 'defend']:
                    print("Queueing defense")
                    self.defend()
                elif action.lower() in ['3', 'bag']:
                    self.backpack.openbag()
                elif action.lower() in ['4', 'dodge']:
